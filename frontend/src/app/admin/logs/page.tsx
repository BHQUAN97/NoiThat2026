...
import type { AppLog, LogLevel, LogStats } from '@/types'

const LEVEL_CONFIG: Record<LogLevel, { icon: typeof AlertCircle; label: string; color: string; bg: string }> = {
...
export default function AdminLogsPage() {
  ...
  const { items: logs, meta, page, loading, error, goToPage, refresh, isEmpty } = useAdminList<AppLog>({
    endpoint: '/logs',
    limit: 50,
    params: { level: level || undefined, search: search || undefined },
  })
  ...
  const clearFilters = () => { setLevel(''); setSearch(''); setSearchInput('') }
    ...
    <div className="py-4">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-body-md text-on-surface-variant">Theo dõi lỗi, cảnh báo và thông tin hệ thống</p>
        </div>
        <div className="flex items-center gap-2">
          {selectedIds.size > 0 && (
            <button
              ...
              disabled={deleting}
              className="flex items-center gap-1.5 rounded-xl bg-error-container px-4 py-2 min-h-[44px] text-label-md text-on-error-container hover:bg-error-container/80 disabled:opacity-50"
            >
          ...
          <button
            ...
            className="flex items-center justify-center rounded-xl p-2.5 min-h-[44px] min-w-[44px] text-error/60 hover:bg-error-container/30 disabled:opacity-30"
      ...
      <div className="mt-6 flex flex-wrap items-center gap-3">
          ...
          {(['', 'error', 'warn', 'info'] as const).map((l) => (
            <button
              ...
              className={cn(
                ...
                level === l ? 'bg-primary-container text-on-primary-container' : 'text-on-surface-variant hover:bg-surface-container-high',
              )}
            >
        ...
        <form onSubmit={handleSearch} className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            className="w-full rounded-xl bg-surface-container py-2 pl-10 pr-4 text-body-sm text-on-surface outline-none focus:ring-2 focus:ring-primary/30"
          />
        </form>
        ...
        <DataStates loading={loading} error={error} isEmpty={isEmpty} onRetry={refresh} emptyMessage="Không có log nào" minHeight="min-h-[20vh]">
          {logs.length > 0 && (
            <div className="mb-2 flex items-center gap-3 px-1">
              <button type="button" onClick={toggleSelectAll} className="flex items-center gap-2 rounded-lg px-2 py-1 text-label-sm text-on-surface-variant hover:bg-surface-container">
                {selectedIds.size === logs.length ? <CheckSquare className="h-4 w-4 text-primary" /> : <Square className="h-4 w-4" />}
          ...
          <div className="space-y-2">
            {logs.map((log) => (
              <LogRow key={log.id} log={log} selected={selectedIds.has(log.id)} onToggle={() => toggleSelect(log.id)} onClick={() => setSelectedLog(log)} />
            ))}
          </div>
        </DataStates>
      </div>
...
function StatCard({ label, value, color }: { label: string; value: number; color: 'error' | 'warning' | 'info' | 'neutral' }) {
  const colorMap = { error: 'bg-error/5 text-error', warning: 'bg-warning/5 text-warning', info: 'bg-info/5 text-info', neutral: 'bg-on-surface/5 text-on-surface' }
  const [bg, text] = colorMap[color].split(' ')
  return (
    <div className={cn('rounded-2xl p-4', bg)}>
      <p className={cn('font-body text-headline-md font-bold', text)}>{formatNumber(value)}</p>
      <p className="mt-0.5 text-body-sm text-on-surface-variant">{label}</p>
    </div>
  )
}

function LogRow({ log, selected, onToggle, onClick }: { log: AppLog; selected: boolean; onToggle: () => void; onClick: () => void }) {
  const config = LEVEL_CONFIG[log.level] || LEVEL_CONFIG.info
  const Icon = config.icon

  return (
    <div className={cn('flex w-full items-start gap-3 rounded-xl p-4 transition-colors', selected ? 'bg-primary-container/20' : 'bg-surface-container-low/50 hover:bg-surface-container-low')}>
      <button type="button" onClick={(e) => { e.stopPropagation(); onToggle() }} className="mt-0.5 shrink-0 p-0.5 text-on-surface-variant hover:text-primary">
      ...
      <button type="button" onClick={onClick} className="flex flex-1 items-start gap-3 text-left">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            ...
            {log.status_code && (
              <span className={cn('shrink-0 rounded-md px-1.5 py-0.5 text-[11px] font-bold', log.status_code >= 500 ? 'bg-error/10 text-error' : log.status_code >= 400 ? 'bg-warning/10 text-warning' : 'bg-surface-container text-on-surface-variant')}>
                {log.status_code}
              </span>
            )}
          </div>
...
function DeleteConfirmDialog({ type, count, level, deleting, onConfirm, onCancel }: { type: 'bulk' | 'all'; count: number; level?: string; deleting: boolean; onConfirm: () => void; onCancel: () => void }) {
  useEffect(() => {
  ...
  const message = type === 'bulk' ? `Xóa ${count} log đã chọn?` : level ? `Xóa tất cả log "${level}"?` : 'Xóa tất cả log?'
  ...
  return (
    <>
      ...
      <div className="fixed inset-x-4 top-1/3 z-[61] mx-auto max-w-sm rounded-2xl bg-surface-container-lowest p-6 shadow-ambient-xl sm:inset-x-auto sm:w-full">
        ...
        <p className="mt-4 text-body-sm text-on-surface-variant">Hành động này không thể hoàn tác.</p>
        <div className="mt-6 flex justify-end gap-3">
          <button onClick={onCancel} disabled={deleting} className="rounded-xl px-5 py-2.5 min-h-[44px] text-label-md text-on-surface-variant hover:bg-surface-container disabled:opacity-50">Hủy</button>
          <button onClick={onConfirm} disabled={deleting} className="rounded-xl bg-error px-5 py-2.5 min-h-[44px] text-label-md text-on-error hover:bg-error/90 disabled:opacity-50">{deleting ? 'Đang xóa...' : 'Xóa'}</button>
        </div>
      </div>
    </>
  )
    ...
    <>
      ...
      <div className="fixed inset-x-4 top-[5%] z-[61] mx-auto max-h-[90vh] max-w-3xl overflow-y-auto rounded-2xl bg-surface-container-lowest shadow-ambient-xl sm:inset-x-auto sm:w-full">
        ...
        <div className="space-y-4 px-6 pb-6">
          <DetailSection label="Message"><p className="text-body-md text-on-surface">{log.message}</p></DetailSection>
          ...
          {log.ip && (
            <DetailSection label="IP / User-Agent">
          ...
          {log.stack_trace && (
            <DetailSection label="Stack Trace">
              <pre className="max-h-[300px] overflow-auto rounded-xl bg-on-surface p-4 text-[12px] leading-relaxed text-surface/90 font-mono">{log.stack_trace}</pre>
            </DetailSection>
          )}
          {log.context && Object.keys(log.context).length > 0 && (
            <DetailSection label="Request Context">
              <pre className="max-h-[200px] overflow-auto rounded-xl bg-surface-container p-4 text-[12px] leading-relaxed text-on-surface font-mono">{JSON.stringify(log.context, null, 2)}</pre>
            </DetailSection>
          )}
        </div>
      </div>
...
function MetaItem({ icon: Icon, label, value }: { icon: typeof Clock; label: string; value: string }) {
  return (
    <div className="rounded-xl bg-surface-container-low p-3">
      <div className="flex items-center gap-1.5 text-[11px] text-on-surface-variant/50"><Icon className="h-3 w-3" />{label}</div>
      <p className="mt-1 text-body-sm font-medium text-on-surface truncate" title={value}>{value}</p>
    </div>
  )
}
